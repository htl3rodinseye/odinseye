 ...
 */
pub fn create_dir_structure(root: String, fs: Vec<&str>) -> Result<(), Error> {
    let env_dir: String = generate_program_path(root); // Generate the program path and return it as String

    if env_dir != "" {
        // If the directory creation succeeded
        for dir in fs {
            // Create the directory structure provided as vector provided by the second parameter
